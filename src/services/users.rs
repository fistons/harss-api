...
use crate::common::{
    email::send_email,
    model::{PagedResult, User, UserRole},
};
...
impl UserService {
    ...
    /// Return the user matching the id
    #[instrument(skip(self))]
    pub async fn get_user_by_email(&self, email: &Secret<String>) -> Result<Option<User>> {
    ...
    pub async fn reset_password(
        ...
        if let Some(user) = user {
            ...
            let registered_token: Option<String> = self
                .redis
                .get()
                .await?
                .get(&key)
            ...
            if let Some(registered_token) = registered_token {
                if registered_token == *token {
                    self.update_user_password(user.id, new_password).await?;

                    self.redis.get().await?.del::<String, usize>(key).await?;
    ...
    #[instrument(skip_all)]
    pub async fn reset_password_request(&self, email: &Secret<String>) -> anyhow::Result<()> {
        ...
        if let Some(user) = user {
            ...
            let reset_token = self
                .generate_and_persist_token(&key, ttl_in_minutes * 60)
                .await?;
    ...
    pub async fn update_user(
        ...
        if let Some(secret_email) = email {
            self.send_confirmation_email(&user, secret_email).await?;
    ...
    pub async fn confirm_email(&self, user_id: i32, token: &Secret<String>) -> anyhow::Result<()> {
        let key = format!("user.{}.confirm-email-token", user_id);
        let registered_token: Option<String> = self.redis.get().await?.get(&key).await?;
    ...
    pub async fn delete_user(&self, user_id: i32) -> anyhow::Result<()> {
        ...
        self.delete_user_redis_keys(user_id).await?;
        info!("Deleted user {}", user_id);
        Ok(())
    }

    #[instrument(skip(self))]
    pub async fn delete_user_redis_keys(&self, user_id: i32) -> anyhow::Result<()> {
            ...
            .iter()
        {
            ...
            self.redis.get().await?.del::<&str, usize>(key).await?;
        }
    ...
    async fn send_confirmation_email(
        ...
        let reset_token = self
            .generate_and_persist_token(&key, ttl_in_days * 86400)
            .await?;
        let data = ResetPasswordData {
            dest_name: &user.username,
            dest_email: email.expose_secret(),
